...
function initializePageContent() {
    ...
    // 初始化播放器
    if (videoUrl) {
        // 检查是否是电影天堂源(source=dyttzy)
        const isDyttSource = sourceCode === 'dyttzy';
        
        // 检查是否是电影天堂域名的链接
        const dyttDomains = ['dytt-cinema.com', 'dyttzyapi.com', 'dyttzy.com'];
        const isDyttLink = dyttDomains.some(domain => videoUrl.includes(domain));
        
        if (isDyttSource || isDyttLink) {
            // 自动复制链接到剪贴板（模拟点击"复制播放链接"按钮）
